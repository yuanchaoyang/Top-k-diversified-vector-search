...
# Train combined word+sentence intent model (two-step: label → train)
python scripts/label_sentences_for_intent.py  # generates ChatGPT prompt, save reply to data/chatgpt_sentence_labels.txt
python scripts/train_sentence_intent.py       # merges word+sentence labels → models/intent_v2/
